...
class Dataloader(object):
    ...
    def obtain_idx(self, idx, option):
        if option == 'train':
            ...
            entry = self.train[table_id] # 序列化的表格，5个例子，每个例子4个要素，PPT里有
        elif option == 'val':
...
class NormalTableDatabase(Dataloader):
    ...
    def get_data(self, idx, option='train', debug=False, with_fields=False):
        ...
        if option in ['train']:
            random.shuffle(entry)
            entry = entry[:self.batch_size]#取前batch_size个，这里正好相等 （5，4）

        for e in entry: 
            e[0] = e[0].lower() # 小写
            e_in = "<SOS> " + e[0] # 对每一行信息的文本描述
            e_out = e[0] + " <EOS>"
            input_fields.append(e[1]) # e[1]本身就是数字，不需要用vocab映射

            seqs_out.append([self.full_vocab.get(_, self.full_vocab['<UNK>']) for _ in e_out.split(' ')])
            # seqs_out中存储每一个sentance为embedding,这里已经变成数字了，16个单词对应16个数字 "word-by-word"
            ...
            # 对每个instance的第三个变量编码，对5个示例e[2]都是一样的，是一句对table信息的描述
            table_orig = []
            table_unk = []
            field = []
            index = []
            lookup = []
            line_no = []
            # i 15行 j5列，这里是遍历cell,每个cell不一定是一个单词,这边主要是处理
            for i in range(len(d)):
