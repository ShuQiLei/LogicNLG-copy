...
class TableInfusing(nn.Module):
    def __init__(self, vocab_size, full_vocab_size, d_word_vec, n_layers, n_head, dropout=0.1):
        ...
        self.embed = nn.Embedding(vocab_size, d_word_vec, padding_idx=0)# 词典长度，向量维度(10105,256)

        self.vocab_size = vocab_size
        self.full_vocab_size = full_vocab_size # 113359 
        ...
        self.enc_stack = nn.ModuleList([
            ...
            for _ in range(n_layers)]) # 输入的是ek

        self.dec_stack = nn.ModuleList([
            DecoderLayer(d_word_vec, d_inner, n_head, d_k, d_v, dropout=dropout)
            for _ in range(n_layers)]) # 三层

        self.post_word_emb = PositionalEmbedding(d_model=d_word_vec)

        self.copy_gate = nn.Sequential(nn.Linear(d_word_vec, 1), nn.Sigmoid()) # 这一层输出用来干嘛

        self.tgt_word_prj = nn.Linear(d_word_vec, vocab_size, bias=False) # 解码的预测输出

    def forward(self, seqs_in, table_in, table_scatters, lookups, line_nos, fields, indexes):
        enc_inp = self.encode(table_in, lookups, line_nos, fields, indexes)    #(5,_,256),_取决于word数量
    ...
    def encode(self, table_in, lookups, line_nos, fields, indexes):
        ...
        out, hidden = self.field_encoder(field_emb)# out(5,5,256)hidden(torch.size[1,5,256],torch.size[1,5,256])
        out = out.transpose(1, 0)# out.shape(5,5,256)

        field_mask = (fields != 0).unsqueeze(-1).float()  # (5,5,1)
        out = out * field_mask   # 将某些维度置为0

        extracted = torch.gather(out, 1, indexes[:, :, None].repeat(1, 1, out.shape[-1])) # (5,3,256)
        # 从乱序中获取索引下的数据
        field_emb = torch.gather(extracted, 1, lookups[:, :, None].repeat(1, 1, extracted.shape[-1])) #(5,94,256)

        line_no_emb = self.embed(line_nos) # row 编码

        word_emb = self.embed(table_in)

        cell_emb = field_emb + line_no_emb + word_emb # ei
    ...
    def decode(self, seqs_in, enc_inp, table_scatters):
        ...
        tgt_emb = self.embed(seqs_in) # ([5, 20, 256])
        dec_inp = tgt_emb + self.post_word_emb(seqs_in) # 位置编码

        src_slf_mask = (table_scatters == 0) # ([5,119])对整个序列化的表格信息做掩码
        tgt_slf_mask = (seqs_in == 0)

        non_pad_mask = get_non_pad_mask(seqs_in) # 计算注意力所以做升维操作([5, 20, 1]),seqs_in = ([5,20])
        ...
        gate = self.copy_gate(dec_inp) # ([5, 20, 1])输出的预测概率
        ...
        full_logits = torch.log(full_prob) # 输出每个word对应full vocab中的概率，用来计算loss
