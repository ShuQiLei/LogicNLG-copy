...
def parse_opt():
    ...
    parser.add_argument('--epoch', default=2, type=int, help="whether to train or test the model")
...
if __name__ == "__main__":
    ...
    if args.do_train:
        ...
        for epoch_idx in range(args.epoch):
            ...
            dataset.shuffle()
            # 网络训练是参数的梯度置为0\计算loss,需要parameter\backward更新梯度\最后是optimizer.step
            for idx in range(0, dataset.train_len()):
                ...
                model.zero_grad() # 每一次batch内数据计算梯度前把前一批训练计算的梯度置0
                optimizer.zero_grad() # 把模型中parameter的参数置为0
                # 训练，什么是输入什么是输出
                logits = model(seqs_in, table_in, table_scatters, lookups, line_nos, fields, indexes) #(5,23,113359)
                ...
                loss.backward()
                optimizer.step()# 根据反向传播的梯度信息更新网络的参数
                # 每every个打印一次日志
